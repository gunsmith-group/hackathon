...
import {call_near_readonly} from "../../utils/near";
...
const GGContractState = ({contractAddress}) => {
    const [contractState, setContractState] = useState(null)

    const getState = async () => {
        const contracts = await call_near_readonly(contractAddress, 'get_contracts');
        const members = await call_near_readonly(contractAddress, 'get_members');
        setContractState({contracts, members})
    }
...
export default GGContractState;